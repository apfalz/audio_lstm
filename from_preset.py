...
with tf.device(tm.device):
    ...
    if tm.use_transpose == True:
        ...
        output_1     = tf.layers.dense(inputs=logits,   units=tm.output_shape[1], kernel_initializer=tf.truncated_normal_initializer(mean=0.0, stddev=tm.weight_stddev))
        output       = tf.layers.dense(inputs=output_1, units=tm.output_shape[1], kernel_initializer=tf.truncated_normal_initializer(mean=0.0, stddev=tm.weight_stddev))
